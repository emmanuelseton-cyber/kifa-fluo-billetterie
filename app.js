// ── Firebase Config ─────────────────────────────────────────
var firebaseConfig = {
  apiKey: "AIzaSyCQ-xGB9A3CQZhuCjuanDnpr3LXw1b5Eqs",
  authDomain: "kifa-fluo-billetterie.firebaseapp.com",
  databaseURL: "https://kifa-fluo-billetterie-default-rtdb.europe-west1.firebasedatabase.app",
  projectId: "kifa-fluo-billetterie",
  storageBucket: "kifa-fluo-billetterie.firebasestorage.app",
  messagingSenderId: "152176000233",
  appId: "1:152176000233:web:201cda0510c3bf568f8478"
};
...
ticketsRef.on('value', function(snapshot) {
  ...
  if (lq && lq.closest('#pane-list.on')) drawList();
});

function dbSave() {
  ticketsRef.set(DB).catch(function(e) {
    notif('Erreur sync Firebase.', 'err');
  });
}
...
function nextNum() {
  ...
  for (i = 0; i < keys.length; i++) {
    p = keys[i].split('-'); n = parseInt(p[p.length-1]);
    if (n > 0) nums.push(n);
  }
...
// ── Dashboard ────────────────────────────────────────────────
function drawDash() {
  var all = Object.values(DB), tot = all.length, ent = 0, i, rec = [], h, t, html = '';
  for (i = 0; i < all.length; i++) { if (all[i].scanned) ent++; }
  var att = tot - ent;
  var p1 = tot ? Math.round(ent/tot*100) : 0;
  var p2 = Math.min(100, Math.round(tot/180*100));
...
function reinit() {
  ...
  DB[ref].scanned = false; DB[ref].scanned_at = null;
  dbSave(); notif(ref+' réinitialisé.','ok');
}

function vider() {
  if (!confirm('Supprimer TOUS les tickets ?')) return;
  DB = {}; dbSave(); notif('Base vidée.','ok');
}
...
function delRow(ref) {
  ...
  delete DB[ref]; dbSave(); notif(ref+' supprimé.','ok');
}
...
function genFromList(ref) {
  if (!DB[ref]) return; nav('gen');
  document.getElementById('gen-sel').value = ref; onSel(); doGen();
}
...
function dlTicket() {
  ...
  if (!c || c.width === 0) { notif('Génère un ticket.','err'); return; }
...
function renderTicket(nom,ref,qrImg) {
  ...
  ctx.fillStyle = NOIR;
  ctx.beginPath(); ctx.moveTo(20,0); ctx.lineTo(W-20,0); ctx.quadraticCurveTo(W,0,W,20);
  ctx.lineTo(W,460); ctx.lineTo(0,460); ctx.lineTo(0,20); ctx.quadraticCurveTo(0,0,20,0); ctx.closePath(); ctx.fill();
  [{x:150,y:140,r:180,c:'#7B00FF',a:.10},{x:500,y:200,r:140,c:'#00FFCC',a:.08},
   {x:300,y:370,r:120,c:'#FF006E',a:.09},{x:90,y:380,r:80,c:'#00CFFF',a:.07},
   {x:560,y:420,r:100,c:'#FFD700',a:.06}].forEach(function(g) {
    var gr = ctx.createRadialGradient(g.x,g.y,0,g.x,g.y,g.r);
    gr.addColorStop(0,ha(g.c,g.a*2)); gr.addColorStop(1,ha(g.c,0));
  ...
  ctx.fillStyle = OR; ctx.font = '700 10px Arial'; ctx.textAlign = 'center'; ctx.letterSpacing = '3px';
  ctx.fillText('KIFA ROOFTOP',300,73); ctx.letterSpacing = '0px';
  ...
  ctx.fillStyle = '#AAAAAA'; ctx.font = '400 14px Arial'; ctx.letterSpacing = '1px';
  ctx.fillText('DES 17H — JUSQU A 01H',300,295); ctx.letterSpacing = '0px';
  ...
  ctx.fillStyle = '#0A0A0A'; ctx.beginPath(); ctx.arc(0,443,20,0,Math.PI*2); ctx.fill();
  ctx.beginPath(); ctx.arc(W,443,20,0,Math.PI*2); ctx.fill();
  ctx.setLineDash([8,6]); ctx.strokeStyle = '#333'; ctx.lineWidth = 1.5;
  ctx.beginPath(); ctx.moveTo(30,443); ctx.lineTo(570,443); ctx.stroke(); ctx.setLineDash([]);
  ctx.fillStyle = '#1A1A1A'; rr(ctx,260,431,80,24,12); ctx.fill();
  ctx.strokeStyle = '#333'; ctx.lineWidth = .8; rr(ctx,260,431,80,24,12); ctx.stroke();
  ctx.fillStyle = '#666'; ctx.font = '700 9px Arial'; ctx.letterSpacing = '2px';
  ctx.fillText('TICKET',300,447); ctx.letterSpacing = '0px';
  ctx.fillStyle = '#0D0D0D'; ctx.fillRect(0,466,W,H-466);
  ctx.textAlign = 'left'; ctx.fillStyle = '#555'; ctx.font = '400 10px Arial'; ctx.letterSpacing = '2px';
  ctx.fillText('N TICKET',80,510); ctx.letterSpacing = '0px';
  ctx.fillStyle = BLANC; ctx.font = '800 17px Arial'; ctx.letterSpacing = '2px';
  ctx.fillText('#'+ref,80,532); ctx.letterSpacing = '0px';
  ctx.fillStyle = ha(VIOLET,0.15); rr(ctx,400,498,170,55,10); ctx.fill();
  ctx.strokeStyle = VIOLET; ctx.lineWidth = 1; rr(ctx,400,498,170,55,10); ctx.stroke();
  ctx.textAlign = 'center'; ctx.fillStyle = '#BB88FF'; ctx.font = '400 9px Arial'; ctx.letterSpacing = '2px';
  ctx.fillText('TARIF ENTREE',485,517); ctx.letterSpacing = '0px';
  ...
  ctx.textAlign = 'left'; ctx.fillStyle = '#555'; ctx.font = '400 9px Arial'; ctx.letterSpacing = '2px';
  ctx.fillText('DETAILS',80,588); ctx.letterSpacing = '0px';
  [['DATE','Mercredi 8 Juillet 2026'],['HEURE','17h00 - 01h00'],['LIEU','KIFA Rooftop, Bingerville'],
   ['ADRESSE','Lagune Ebrie, Abidjan CI'],['PARTICIPANT',nom]].forEach(function(row,i) {
  ...
  ctx.textAlign = 'center'; ctx.fillStyle = '#444'; ctx.font = '400 9px Arial'; ctx.letterSpacing = '2px';
  ctx.fillText('QR CODE ACCES',300,770); ctx.letterSpacing = '0px';
  ...
  ctx.fillStyle = '#333'; ctx.font = '400 8px Arial'; ctx.letterSpacing = '.5px';
  ctx.fillText('KIFA ROOFTOP - BINGERVILLE - LAGUNE EBRIE - ABIDJAN',300,H-10);
}
